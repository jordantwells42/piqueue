...
export default function DateRange({id}:{id: string }){
    ...
    if (!task){
        ...
        const { start, end, progress } = task
            return (
                <div className='flex flex-row w-full md:w-3/4 lg:w-1/2 justify-between text-center items-center text-black font-bold'>
                ...
                {<h1 className="text-pico-black font-bold text-center ">{Math.round(progress*100)}%</h1>}
                <div className="w-1/4 m-2 h-0 border-b-2 border-pico-dark-blue rounded-full"></div>
